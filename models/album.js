...
module.exports.adressePhoto1 = function(callback) {
    db.getConnection(function(err, connexion) {
        if (!err) {
            var sql = "SELECT VIP_NOM, VIP_PRENOM, v.VIP_NUMERO, PHOTO_ADRESSE FROM photo p JOIN vip v ON p.VIP_NUMERO = v.VIP_NUMERO WHERE PHOTO_NUMERO = 1 ORDER BY VIP_NOM LIMIT 12 OFFSET 12";
...
/**
 * Cette fonction récupère 12 photos 1 de chaque vip, en fonction du rang passé en paramètre
 */
module.exports.photo1tab = function(callback) {
    db.getConnection(function(err, connexion) {
        if (!err) {
            var sql = "SELECT VIP_NOM, VIP_PRENOM, v.VIP_NUMERO, PHOTO_ADRESSE FROM photo p JOIN vip v ON p.VIP_NUMERO = v.VIP_NUMERO WHERE PHOTO_NUMERO = 1 ORDER BY VIP_NOM LIMIT 12 OFFSET 12";
            connexion.query(sql, callback);
